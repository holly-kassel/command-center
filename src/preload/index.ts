...
const githubApi = {
  ...
  setTriageStatus: (
    ...
    status: TriageStatus
  ): Promise<NotificationTriageData> =>
  ...
  getTriageSortOrder: (): Promise<string[]> => ipcRenderer.invoke('github:getTriageSortOrder'),
...
const transcriptionApi = {
  ...
  summarizeMeeting: (input: MeetingSummaryInput): Promise<MeetingNotes> =>
    ipcRenderer.invoke('transcription:summarizeMeeting', input),
  ...
  reviewAction: (
    ...
    ipcRenderer.invoke('transcription:reviewAction', meetingId, actionId, reviewStatus),
  updateNotes: (
    meetingId: string,
    notes: MeetingNotes,
    context: MeetingNotesUpdateContext
  ): Promise<SavedMeeting> =>
    ipcRenderer.invoke('transcription:updateNotes', meetingId, notes, context),
  getMeetings: (): Promise<SavedMeeting[]> => ipcRenderer.invoke('transcription:getMeetings'),
  getMeeting: (meetingId: string): Promise<SavedMeeting | null> =>
    ipcRenderer.invoke('transcription:getMeeting', meetingId),
  syncTeamsTranscript: (meetingId: string): Promise<SavedMeeting> =>
    ipcRenderer.invoke('transcription:syncTeamsTranscript', meetingId),
  saveDraft: (meeting: SavedMeeting): Promise<MeetingDraft> =>
    ipcRenderer.invoke('transcription:saveDraft', meeting),
  getDraft: (): Promise<MeetingDraft | null> => ipcRenderer.invoke('transcription:getDraft'),
  deleteDraft: (meetingId?: string): Promise<boolean> =>
    ipcRenderer.invoke('transcription:deleteDraft', meetingId),
  ...
  onMeetingUpdate: (callback: (meeting: SavedMeeting) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, meeting: SavedMeeting): void =>
      callback(meeting)
    ipcRenderer.on('sync:meeting', handler)
    return () => ipcRenderer.removeListener('sync:meeting', handler)
  }
}
...
const decisionEvalApi = {
  ...
  invalidateCache: (): Promise<void> => ipcRenderer.invoke('decisionEval:invalidateCache')
}
...
const chatApi = {
  ...
  getNudgeConfig: (): Promise<NudgeConfig> => ipcRenderer.invoke('chat:get-nudge-config'),
  ...
  onStreamDone: (callback: (data: { messageId: string }) => void): (() => void) => {
    const handler = (_e: Electron.IpcRendererEvent, data: { messageId: string }): void =>
      callback(data)
